...
echo "Copy data to HDFS  block size = for non-split"

#$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -D dfs.block.size=134217728 -copyFromLocal ~/log/templog1 input/a
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -D dfs.block.size=134217728 -copyFromLocal ~/music/trainpreprocessed_test.txt input/a
...
time $HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar ~/music/musicanalyzer.jar MusicAnalyzer input output

echo "Check output files after PC.. but i remove the old output data first"
rm -r ~/music/output
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -copyToLocal output ~/music/output
